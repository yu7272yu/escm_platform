...
from project_platform.settings import Config, SECRET_KEY
...
def migrate_db():
    try:
        os.system('python3 project_platform/manage.py makemigrations user_manage app_manage auth_code_manage' )
        os.system('python3 project_platform/manage.py migrate' )
        db = pymysql.connect(
            host=  Config.mysql_host,  # 连接名称，默认127.0.0.1
            user= Config.mysql_user,  # 用户名
            passwd= Config.mysql_password ,  # 密码
